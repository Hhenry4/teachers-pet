    ...
    try {
        ...
        const fetchPromises = coursesList.map(async (course) => {
            ...
            try {
                ...
                subs.forEach(s => {
                    submissionsMap[s.courseWorkId] = {
                        state: s.state,
                        assignedGrade: s.assignedGrade !== undefined ? s.assignedGrade : null,
                        draftGrade: s.draftGrade !== undefined ? s.draftGrade : null
                    };
                });
            } catch (e) {
            ...
            const worksAssigned = works.map(w => {
                const sub = submissionsMap[w.id] || {};
                return {
                    id: w.id,
                    courseId: course.id,
                    courseName: course.name,
                    title: w.title,
                    maxPoints: w.maxPoints !== undefined ? w.maxPoints : null,
                    dueDate: w.dueDate ? new Date(Date.UTC(w.dueDate.year, w.dueDate.month - 1, w.dueDate.day, w.dueTime?.hours ?? 23, w.dueTime?.minutes ?? 59)) : null,
...
function calculateLogic(assignment) {
    ...
    if (assignment.dueDate) {
        ...
        if (isMissing) {
            priorityVal = 2;
            priorityLabel = 'High Priority - Late';
            priorityClass = 'badge-high';
            cardClass = 'priority-high';
        } else if (daysDiff >= 0 && daysDiff <= 2) { // Within 48 hours = High
            priorityVal = 2;
            priorityLabel = 'High';
            priorityClass = 'badge-high';
            cardClass = 'priority-high';
        } else if (daysDiff > 2 && daysDiff <= 7) { // Within a week = Medium (bump up 1 priority)
            priorityVal = 1;
...
function renderAssignments() {
    ...
    // Progress bar - new assignments that became due this week
    const progressContainer = document.getElementById('assignments-progress');
    if (progressContainer) {
        ...
        // Only count assignments whose due date falls within THIS week
        // (excludes anything that was already overdue before the week started)
        const thisWeek = filtered.filter(a => a.dueDate && a.dueDate >= weekStart && a.dueDate < weekEnd);
        const completedThisWeek = thisWeek.filter(a => a.state === 'TURNED_IN' || a.state === 'RETURNED').length;
        ...
        if (totalThisWeek > 0) {
            progressContainer.innerHTML = `
                <div class="week-progress-label">
                    <span>📅 This Week</span>
                    <span>${completedThisWeek}/${totalThisWeek} completed</span>
                </div>
    ...
    enhanced.forEach((assignment, index) => {
        ...
        if (assignment.dueDate) {
            const isSameDay = assignment.dueDate.getFullYear() === now2.getFullYear() &&
                              assignment.dueDate.getMonth() === now2.getMonth() &&
                              assignment.dueDate.getDate() === now2.getDate();
            if (isSameDay) {
                const timeStr = assignment.dueDate.toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit', hour12: true });
                dateStr = `⚠️ Due Today at ${timeStr}`;
            } else {
                dateStr = assignment.dueDate.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', hour12: true });
            }
        }
        ...
        const isTurnedIn = assignment.state === 'TURNED_IN' || assignment.state === 'RETURNED';
        card.className = `assignment-card ${pInfo.cardClass} slide-up ${staggerClass}${isTurnedIn ? ' card-completed' : ''}`;
        ...
        card.innerHTML = `
            <div class="card-header">
                ...
                <div class="badge ${pInfo.priorityClass}">${pInfo.priorityLabel} Priority</div>
            </div>
            ${isTurnedIn ? '<div class="completed-watermark">✓</div>' : ''}
...
function renderArchive() {
    ...
    if (done.length === 0) {
        list.innerHTML = '<p class="text-muted" style="text-align:center; padding: 2rem; font-size: 1.1rem;">No completed assignments yet.</p>';
        return;
    }

    done.forEach((a, index) => {
        const staggerClass = `stagger-${Math.min(index % 5 + 1, 5)}`;
        const dateStr = a.dueDate ? a.dueDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'No due date';

        let gradeHTML = '';
        const grade = a.assignedGrade !== undefined && a.assignedGrade !== null ? a.assignedGrade : null;
        const maxPts = a.maxPoints !== null ? a.maxPoints : null;

        if (grade !== null) {
            const pct = maxPts ? Math.round((grade / maxPts) * 100) : null;
            const gradeColor = pct === null ? '#aaa' : pct >= 90 ? '#52c41a' : pct >= 70 ? '#faad14' : '#ff4d4f';
            gradeHTML = `
                <div class="archive-grade" style="color: ${gradeColor}">
                    <span class="grade-score">${grade}${maxPts !== null ? `/${maxPts}` : ''}</span>
                    ${pct !== null ? `<span class="grade-pct">${pct}%</span>` : ''}
                </div>`;
        } else if (a.state === 'RETURNED') {
            gradeHTML = `<div class="archive-grade" style="color: var(--text-muted)"><span class="grade-score">Returned</span></div>`;
        } else {
            gradeHTML = `<div class="archive-grade" style="color: var(--text-muted)"><span class="grade-score">Not graded</span></div>`;
        }

        const item = document.createElement('div');
        item.className = `archive-item slide-up ${staggerClass}`;
        item.style.cursor = 'pointer';
        item.innerHTML = `
            <div class="archive-check">✓</div>
            <div class="archive-info">
                <div class="archive-course">${a.courseName}</div>
                <div class="archive-title">${a.title}</div>
                <div class="archive-date">${dateStr}</div>
            </div>
            ${gradeHTML}
        `;
        item.addEventListener('click', () => window.open(a.url, '_blank'));
