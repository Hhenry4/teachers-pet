...
function generateAndRenderRoutine() {
    ...
    if (actionable.length === 0) {
        container.innerHTML = '<div class="routine-summary">You have no pending assignments! <br><br>Enjoy your free time. 🎉</div>';
        return;
    }
    ...
    let scheduleHTML = `
        <div class="routine-progress-container">
            <div class="routine-progress-fill" id="routine-progress-fill"></div>
            <div class="routine-progress-text" id="routine-progress-text">0% Completed</div>
        </div>
        <div class="routine-summary" style="margin-bottom: 2rem;">Based on urgency, here is your optimized Smart Study Routine starting now:</div>
    `;
    ...
    actionable.forEach((task, index) => {
        ...
        scheduleHTML += `
            <div class="timeline-item slide-up ${staggerClass} routine-check-item">
                ...
                <div class="timeline-content" style="border-left: 4px solid var(--${task.pInfo.priorityVal === 2 ? 'priority-high' : (task.pInfo.priorityVal === 1 ? 'priority-medium' : 'priority-low')});">
                    <div class="timeline-task-header">
                        <div style="font-size: 0.8rem; color: var(--text-muted);">${startTimeStr}</div>
                        <button class="check-btn" title="Mark step as done">
    ...
    // Attach Event Listeners and Progress State Logic
    const routineItems = container.querySelectorAll('.routine-check-item');
    ...
    function updateProgress() {
        ...
        // Setup Active Task Pulse
        let foundNext = false;
        routineItems.forEach(item => {
            item.classList.remove('active-task');
            if (!item.classList.contains('completed') && !foundNext) {
