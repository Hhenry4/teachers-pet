...
async function fetchClassroomData() {
    try {
        ...
        const fetchPromises = coursesList.map(async (course) => {
            ...
            let announcements = [];
            try {
                const annData = await fetchWithToken(`https://classroom.googleapis.com/v1/courses/${course.id}/announcements`);
...
function renderAnnouncements() {
    ...
    sorted.forEach((ann, index) => {
        const staggerClass = `stagger-${Math.min(index % 5 + 1, 5)}`;
        const dateStr = ann.updateTime.toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute:'2-digit' });
...
function generateAndRenderRoutine() {
    ...
    let scheduleHTML = `<div class="routine-summary">Based on urgency, here is your optimized Smart Study Routine starting now:</div>`;
    ...
    actionable.forEach((task, index) => {
        ...
        const startTimeStr = timeAccumulator.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' });
        ...
        if (index < actionable.length - 1) {
            const breakTimeStr = timeAccumulator.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' });
            scheduleHTML += `
                <div class="timeline-item slide-up ${staggerClass}">
                    ...
                    <div class="timeline-content" style="opacity: 0.7; padding: 1rem;">
                        <div style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 0.2rem;">Start at ${breakTimeStr}</div>
